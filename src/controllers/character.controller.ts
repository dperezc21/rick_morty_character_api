...
import {CHARACTER_STRATEGY_CONTEXT} from "../services/character-strategy.service";
...
export class CharacterController {
    async getAllCharacters(filter: string, type: string): Promise<Character[]> {
        const charactersByStatus = cacheService.getValue(filter);
        if(charactersByStatus) return JSON.parse(charactersByStatus);
        const characters: Character[] = await CHARACTER_STRATEGY_CONTEXT.get(type).getCharacters(filter);
    ...
    async getAllCharactersByOrigin(filter: string, type: string): Promise<Character[]> {
        const charactersByStatus = cacheService.getValue(filter);
        if(charactersByStatus) return JSON.parse(charactersByStatus);
        const characters: Character[] = await CHARACTER_STRATEGY_CONTEXT.get(type).getCharacters(filter);
        if(characters.length && characters?.length > CHARACTER_NUMBER_MIN) return characters;
    ...
    async saveCharacterData(characters: Character[]): Promise<boolean> {
        for (const charactersToSaveElement of characters) {
            const characterSaved: Character = await characterService.saveCharacter(charactersToSaveElement);
